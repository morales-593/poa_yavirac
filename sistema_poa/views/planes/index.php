                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                ...
                                                <button class="btn btn-warning btn-sm"
                                                    onclick="abrirSeguimiento(<?= $p['id_plan'] ?>)" title="Seguimiento"
                                                    <?= !Plan::tieneElaboracion($p['id_plan']) ? 'disabled' : '' ?>>
                                                    <i class="fas fa-chart-line"></i> Seguimiento
                                                </button>
                                                <button class="btn btn-success btn-sm"
                                                    onclick="abrirEjecucion(<?= $p['id_plan'] ?>)" title="Ejecución"
                                                    <?= !Plan::tieneElaboracion($p['id_plan']) ? 'disabled' : '' ?>>
...
<script>
    // Agregar estas funciones al final del archivo planes/index.php
function abrirSeguimiento(id_plan) {
    fetch(`index.php?action=modalSeguimiento&id_plan=${id_plan}`)
        .then(response => response.text())
        .then(html => {
            document.getElementById('contenedorElab').innerHTML = html;
            const modal = new bootstrap.Modal(document.getElementById('modalElab'));
            modal.show();
        })
        .catch(error => {
...
function abrirEjecucion(id_plan) {
    fetch(`index.php?action=modalEjecucion&id_plan=${id_plan}`)
        .then(response => response.text())
        .then(html => {
            document.getElementById('contenedorElab').innerHTML = html;
            const modal = new bootstrap.Modal(document.getElementById('modalElab'));
